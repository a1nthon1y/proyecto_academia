              ...
              <>
                ...
                <ul className="md:hidden divide-y divide-slate-100">
                  {filtrados.map((tutor) => (
                    <li
                      key={tutor.id}
                      className={`p-4 ${!tutor.activo ? 'bg-slate-50/60 opacity-75' : ''}`}
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          <div className={`font-semibold truncate ${tutor.activo ? 'text-navy-900' : 'text-slate-600'}`}>
                      ...
                      <div className="mt-3 space-y-1 text-sm text-slate-600">
                        <a href={tutor.email ? `mailto:${tutor.email}` : undefined} className="flex items-center gap-2 truncate">
                          <Mail className="h-3.5 w-3.5 flex-shrink-0 text-slate-400" />
                          <span className="truncate">{tutor.email || '—'}</span>
                        </a>
                        <a href={tutor.telefono ? `tel:${tutor.telefono}` : undefined} className="flex items-center gap-2">
                          <Phone className="h-3.5 w-3.5 flex-shrink-0 text-slate-400" />
                          <span>{tutor.telefono || '—'}</span>
                        </a>
                        {(tutor.ciudad || tutor.distrito) && (
